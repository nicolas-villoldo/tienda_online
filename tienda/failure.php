...
include("./admin/pedido.php");

$collection_id = $_GET['collection_id'] ?? null;
$status = $_GET['status'] ?? null;
$external_reference = $_GET['external_reference'] ?? null;

if ($status === "failure" && $external_reference) {
    $sql = "UPDATE pedidos SET estado='fallido', pago_id='$collection_id' WHERE id='$external_reference'";
    if ($conexion->query($sql)) {
        $mensaje = "❌ El pago del pedido #$external_reference fue rechazado.";
    } else {
